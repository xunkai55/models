...
flags.DEFINE_string(
    ...
    short_name='bsv',
    default='v1',
    help=flags_core.help_wrap('Beam Search Version'))
...
class BeamSearchState(object):
  """Object storing the state of the beam search loop."""

  def __init__(self,
               cur_index=None,
               alive_seq=None,
               alive_log_probs=None,
               alive_cache=None,
               finished_seq=None,
    ...
    Args:
      ...
      alive_seq: Top sequences that are alive for each batch item. Alive
        sequences are ones that have not generated an EOS token. Sequences that
        reach EOS are marked as finished and moved to the FINISHED_SEQ tensor.
        Has shape [batch_size, beam_size, CUR_INDEX + 1]
      alive_log_probs: Log probabilities of each alive sequence. Shape
        [batch_size, beam_size]
      alive_cache: Dictionary of cached values for each alive sequence. The
        cache stores the encoder output, attention bias, and the decoder
        attention output from the previous iteration.
      finished_seq: Top finished sequences for each batch item. Has shape
        [batch_size, beam_size, CUR_INDEX + 1]. Sequences that are shorter than
        CUR_INDEX + 1 are padded with 0s.
      finished_scores: Scores for each finished sequence. Score = log
        probability / length norm Shape [batch_size, beam_size]
      finished_flags: Flags indicating which sequences in the finished sequences
        are finished. At the beginning, all of the sequences in FINISHED_SEQ are
        filler values. True -> finished sequence, False -> filler. Shape
        [batch_size, beam_size]
    """
    self.cur_index = cur_index
    self.alive_seq = alive_seq
    self.alive_log_probs = alive_log_probs
    self.alive_cache = alive_cache
    self.finished_seq = finished_seq
    self.finished_scores = finished_scores
    self.finished_flags = finished_flags
...
class SequenceBeamSearchV2(v1.SequenceBeamSearch):
  ...
  def search(self, initial_ids, initial_cache):
    ...
    finished_seq, finished_scores = self._get_final_seq_and_scores(state)
  ...
  def _continue_search(self, state):
    ...
    Returns:
      ...
      terminate.
    """
    not_at_max_decode_length = tf.less(state.cur_index, self.max_decode_length)

    # Calculate largest length penalty (the larger penalty, the better score).
    max_length_norm = _length_normalization(self.alpha, self.max_decode_length)
    # Get the best possible scores from alive sequences.
    best_alive_scores = state.alive_log_probs[:, 0] / max_length_norm

    # Compute worst score in finished sequences for each batch element.
    # Set filler scores to zero.
    # TODO
    state.finished_scores *= tf.compat.v1.to_float(state.finished_flags)
    lowest_finished_scores = tf.reduce_min(state.finished_scores, axis=1)

    # If there are no finished sequences in a batch element, then set the lowest
    # finished score to -INF for that element.
    finished_batches = tf.reduce_any(state.finished_flags, 1)
    lowest_finished_scores += (1. - tf.compat.v1.to_float(finished_batches)) * -INF
  ...
  def _search_step(self, state):
    ...
    Returns:
      BeamSearchState: new state.
    """
    # Grow alive sequences by one token.
    new_seq, new_log_probs, new_cache = self._grow_alive_seq(state)

    # Collect top beam_size alive sequences
    (top_alive_seq, top_alive_log_probs,
     top_alive_cache) = self._get_new_alive_state(new_seq, new_log_probs,
                                                  new_cache)

    # Combine newly finished sequences with existing finished sequences, and
    # collect the top k scoring sequences.
    (top_finished_seq, top_finished_scores,
     top_finished_flags) = self._get_new_finished_state(state, new_seq,
                                                        new_log_probs)

    new_state = BeamSearchState(
        cur_index=state.cur_index + 1,  # Increment loop index
        alive_seq=top_alive_seq,
        alive_log_probs=top_alive_log_probs,
        alive_cache=top_alive_cache,
        finished_seq=top_finished_seq,
  ...
  def _grow_alive_seq(self, state):
    ...
    flat_ids = _flatten_beam_dim(state.alive_seq)  # [batch_size * beam_size]
    flat_cache = nest.map_structure(_flatten_beam_dim, state.alive_cache)

    tf.logging.info('++++alive_cache')
    for ln, block in state.alive_cache.items():
      tf.logging.info('      %s', ln)
      if isinstance(block, dict):
        for k, v in block.items():
          tf.logging.info('        %s %s', k, v.shape)
      else:
        tf.logging.info('        %s %s', ln, block.shape)

    flat_logits, flat_cache = self.symbols_to_logits_fn(flat_ids,
                                                        state.cur_index,
                                                        flat_cache)
    tf.logging.info('flat_ids shape: %s', flat_ids.shape)
    # Unflatten logits to shape [batch_size, beam_size, vocab_size]
    tf.logging.info('flat_logits shape: %s', flat_logits.shape)

    logits = _unflatten_beam_dim(flat_logits, self.batch_size, self.beam_size)

    tf.logging.info('logits shape: %s', logits.shape)
    tf.logging.info('++++flat_cache', flat_cache)
    for ln, block in flat_cache.items():
      tf.logging.info('      %s', ln)
      if isinstance(block, dict):
        for k, v in block.items():
          tf.logging.info('        %s %s', k, v.shape)
        ...
        tf.logging.info('        %s %s', ln, block.shape)
    tf.logging.info('batch_size: %s, beam_size: %s', self.batch_size,
                    self.beam_size)
    ...
    topk_seq, new_cache = _gather_beams([state.alive_seq, new_cache],
  ...
  def _get_new_alive_state(self, new_seq, new_log_probs, new_cache):
    ...
    new_log_probs += tf.compat.v1.to_float(new_finished_flags) * -INF

    top_alive_seq, top_alive_log_probs, top_alive_cache = _gather_topk_beams(
        [new_seq, new_log_probs, new_cache], new_log_probs, self.batch_size,
        self.beam_size)
    return top_alive_seq, top_alive_log_probs, top_alive_cache
  ...
  def _get_new_finished_state(self, state, new_seq, new_log_probs):
    ...
    length_norm = _length_normalization(self.alpha, state.cur_index + 1)
    new_scores = new_log_probs / length_norm

    # Set the scores of the still-alive seq in new_seq to large negative values.
    new_finished_flags = tf.equal(new_seq[:, :, -1], self.eos_id)
    new_scores += (1. - tf.compat.v1.to_float(new_finished_flags)) * -INF

    # Combine sequences, scores, and flags.
    finished_seq = tf.concat([finished_seq, new_seq], axis=1)
    finished_scores = tf.concat([state.finished_scores, new_scores], axis=1)
    finished_flags = tf.concat([state.finished_flags, new_finished_flags],
                               axis=1)
    ...
    return top_finished_seq, top_finished_scores, top_finished_flags

  def _get_final_seq_and_scores(self, state):
    # Account for corner case where there are no finished sequences for a
    # particular batch item. In that case, return alive sequences for that batch
    # item.
    is_finished = tf.reduce_any(state.finished_flags, 1)
    finished_seq = tf.where(is_finished, state.finished_seq, state.alive_seq)
    finished_scores = tf.where(is_finished, state.finished_scores,
                               state.alive_log_probs)
    return finished_seq, finished_scores
...
def sequence_beam_search(
  ...
  tf.logging.info('beam_search_version version: %s',
                  tf.flags.FLAGS.beam_search_version)
  if tf.flags.FLAGS.beam_search_version == 'v1':
    sbs = v1.SequenceBeamSearch(symbols_to_logits_fn, vocab_size, batch_size,
                                beam_size, alpha, max_decode_length, eos_id)
    return sbs.search(initial_ids, initial_cache)
  elif tf.flags.FLAGS.beam_search_version == 'v2':
    sbs = SequenceBeamSearchV2(symbols_to_logits_fn, vocab_size, batch_size,
                               beam_size, alpha, max_decode_length, eos_id)
    return sbs.search(initial_ids, initial_cache)
  else:
    raise ValueError('Not supported beam_search_version: {}'.format(
        tf.flags.FLAGS.beam_search_version))
...
def _length_normalization(alpha, length):
  ...
  return tf.pow(((5. + tf.compat.v1.to_float(length)) / 6.), alpha)
...
def _unflatten_beam_dim(tensor, batch_size, beam_size):
  ...
  tf.logging.info('  original shape: %s', shape)
  tf.logging.info('  to shape: %s', new_shape)
