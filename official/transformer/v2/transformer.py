...
class TransformerV2(tf.keras.Model):
  ...
  def __init__(self, params, train, name=None):
    ...
    Args:
      ...
      name: TODO
    """
    super(TransformerV2, self).__init__(name=name)
    self.train = train
    self.params = params
...
class LayerNormalization(tf.keras.layers.Layer):
  ...
  def __init__(self, hidden_size):
    ...
    self.scale = self.add_weight(
        "layer_norm_scale", shape=[self.hidden_size], dtype="float32",
        initializer=tf.ones_initializer())
    self.bias = self.add_weight(
        "layer_norm_bias", shape=[self.hidden_size], dtype="float32",
